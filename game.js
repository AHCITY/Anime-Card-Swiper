...
wallpaperUpload.addEventListener('change', async (e) => {
    ...
    try {
        const filename   = `wallpapers/${currentUser.uid}/${Date.now()}_${file.name.replace(/[^a-zA-Z0-9._-]/g, '_')}`;
        const sRef       = ref(storage, filename);
        const snapshot   = await uploadBytes(sRef, file);
        const downloadUrl= await getDownloadURL(snapshot.ref);
        await WallpaperDB.save(downloadUrl, file);
        const entry = {
            url:       downloadUrl,
        ...
        if (wallpaperLibrary.length > 20) wallpaperLibrary = wallpaperLibrary.slice(0, 20);
        customWallpaperUrl = downloadUrl;
        wallpaperSetting   = 'custom';
        applyWallpaper();
        showNotification('✅ Wallpaper saved to library!', 'success');
        await savePlayerData(true);
    } catch (err) {
...
function openInventory() {
    // Allow deck changes between matches (when hand hasn't been dealt yet for next match)
    ...
    const stacked = Array.from(cardMap.values()).sort((a, b) => b.power - a.power);
